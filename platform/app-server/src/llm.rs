...
- observationScore is awarded to creator_player_id / creator_name for how that player handled their own dragon during Phase 1
- careScore is awarded to current_owner_id / current_owner_name for how that player handled this other dragon during Phase 2
- creativityScore is informational only; use it for the clarity, specificity, and usefulness of the written observations and handover notes

Judge the player cycle, not the visual character design:
- actual_active_time, actual_favorite_food, actual_favorite_play, and actual_sleep_rate are the hidden habits generated by the game
- creator_instructions is character/design text from setup; it is context only and must NOT be treated as Phase 1 care instructions
- discovery_observations and handover_tags are the Phase 1 sitter's evidence and recommendations
- phase2_actions are only the next sitter's Phase 2 care actions for this dragon
- first score whether the Phase 1 sitter discovered and transferred useful, relevant instructions
- then score whether the Phase 2 sitter followed the received handover and actual hidden habits
- if the received handover was irrelevant, vague, or contradictory, say so in careFeedback and judge whether the sitter adapted reasonably anyway
...
- Ignore visual appearance and popularity. Do not reward style, looks, names, or votes.
  - If evidence is weak, contradictory, or missing, score conservatively instead of guessing.
  - Return observationFeedback for the Phase 1 sitter only.
  - Return careFeedback for the Phase 2 sitter only.
  - Keep feedback as a one-sentence combined overview for backward compatibility.

Scoring rubric:
- observationScore (0-100)
  - 40 points: how accurately discovery_observations and handover_tags match actual_active_time, actual_favorite_food, actual_favorite_play, and actual_sleep_rate
  - 25 points: whether the handover gives the next sitter concrete, actionable recommendations
  - 20 points: completeness across activity rhythm, food, play, rest, and any observed reactions
...
- creativityScore (0-100)
  - informational only; evaluate how memorable, concise, and useful the written observations and handover are
  ...
  "dragonEvaluations": [
    {
      ...
      "careScore": 74,
      "creativityScore": 68,
      "observationFeedback": "The Phase 1 sitter identified useful habits and handed over specific recommendations.",
      "careFeedback": "The Phase 2 sitter mostly followed the relevant handover but made one avoidable food mistake.",
      "feedback": "The Phase 1 handover was useful, while Phase 2 care mostly followed it with one avoidable mistake."
    }
  ]
}"#
}
...
- For each observationFeedback, evaluate only how creator_name discovered hidden habits and handed over relevant instructions from discovery_observations and handover_tags.
- For each careFeedback, evaluate only how current_owner_name followed or adapted to the received handover using phase2_actions and finalStats.
- In careFeedback, explicitly mention whether the received handover was relevant, partly relevant, irrelevant, vague, or contradictory.
- For each combined feedback, give a short player-cycle overview across Phase 1 handover and Phase 2 care.
...
struct RawDragonEvaluation {
    ...
    #[serde(default)]
    observation_feedback: String,
    #[serde(default)]
    care_feedback: String,
    feedback: String,
}
...
fn parse_judge_response(text: &str) -> Result<LlmJudgeEvaluation, String> {
    ...
    Ok(LlmJudgeEvaluation {
        ...
        dragon_evaluations: raw
            ...
            .into_iter()
            .map(|d| {
                let observation_feedback = if d.observation_feedback.trim().is_empty() {
                    d.feedback.clone()
                } else {
                    d.observation_feedback
                };
                let care_feedback = if d.care_feedback.trim().is_empty() {
                    d.feedback.clone()
                } else {
                    d.care_feedback
                };
...
fn validate_judge_response(
        ...
        .enumerate()
    {
        ...
        if actual.observation_feedback.trim().is_empty() {
            return Err(format!(
                "dragonEvaluations[{index}].observationFeedback must not be empty"
            ));
        }
        if actual.care_feedback.trim().is_empty() {
            return Err(format!(
                "dragonEvaluations[{index}].careFeedback must not be empty"
            ));
        }
    }
    ...
    fn parse_judge_response_handles_clean_json() {
        let json = r#"{
            ...
            "dragonEvaluations": [{
                ...
                "careScore": 85,
                "creativityScore": 70,
                "observationFeedback": "Useful Phase 1 handover.",
                "careFeedback": "Phase 2 followed the relevant handover.",
...
```"#;
    ...
    fn validate_judge_response_rejects_missing_dragon_rows() {
        ...
        let json = r#"{
            ...
            "dragonEvaluations": [{
                ...
                "careScore": 80,
                "creativityScore": 50,
                "observationFeedback": "Observation feedback.",
    ...
    fn validate_judge_response_accepts_exact_bundle_order() {
            ...
            "dragonEvaluations": [
                {
                    ...
                    "careScore": 80,
                    "creativityScore": 50,
                    "observationFeedback": "Good handover quality.",
                    "careFeedback": "Relevant handover was followed.",
                ...
                },
                {
                    ...
                    "careScore": 75,
                    "creativityScore": 55,
                    "observationFeedback": "Clear observations.",
                    "careFeedback": "Mostly correct care decisions from a partly relevant handover.",
        ...
        assert_eq!(
            evaluation.dragon_evaluations[1].care_feedback,
            "Mostly correct care decisions from a partly relevant handover."
        );
    }
